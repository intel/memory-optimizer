...
unsigned long long g_idlebufsize = 0;
unsigned short *g_refs_count = NULL;
unsigned long long g_offset = 0, g_size = 0;
unsigned long long g_start_pfn = 0, g_num_pfn = 0;
...
int output_refs(const char *output_file)
{
	...
	for (pfn = 0; pfn < g_num_pfn; pfn ++) {
		fprintf(file, "0x%016llx  %d\n", g_start_pfn + pfn, g_refs_count[pfn]);
	}
	fclose(file);
	return 0;
}
		...
		for (pfn = 0; pfn < 64; pfn ++) {
			if (!(idlebits & (1ULL << pfn))) {
				...
				debug_printf("pfn 0x%llx refs_count 0x%llx\n",
			    		base_pfn + pfn, g_refs_count[base_pfn + pfn]);
			}
		}
...
int main(int argc, char *argv[])
{
	...
	printf("Watching the %s to calculate page reference count "
		   "every %.2f seconds for %d times.\n"
		   "The result will be saved in %s. \n",
		   bitmap_file, interval, loop, output_file);
